...
-   **URL Processing Path**:
    -   If the input is classified as a URL:
        ...
        -   **AI Parsing (Gemini)**: The extracted textual content (title, ingredients text, instructions text) is then passed to a Generative AI model (Google Gemini via `gemini-1.5-flash-latest`). A specific prompt guides the AI to parse these text sections into a structured JSON object (`CombinedParsedRecipe` type defined in `api/routes/recipes.ts`). This includes structuring ingredients with amounts and units, generating ingredient substitution suggestions, and formatting instructions.
...
-   **Raw Text Processing Path**:
    -   If the input is classified as raw text (i.e., not a URL):
        -   **AI Parsing (Gemini)**: The entire raw text is passed directly to the Google Gemini AI model (`gemini-1.5-flash-latest`).
