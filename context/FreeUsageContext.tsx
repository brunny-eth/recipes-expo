...
export const FreeUsageProvider = ({ children }: PropsWithChildren) => {
  ...
  const resetFreeRecipeUsage = useCallback(async () => {
    console.log('[FreeUsageContext] Attempting to reset free recipe usage locally and in Supabase.');
    ...
    if (user) {
        ...
        if (error) {
            console.error('[FreeUsageContext] Error updating user metadata on Supabase:', error.message);
        } else {
            console.log('[FreeUsageContext] Successfully reset has_used_free_recipe in Supabase user_metadata.');
        }
    } else {
        console.warn('[FreeUsageContext] No user session found. Skipping Supabase metadata update.');
    }
