...
export async function parseAndCacheRecipe(
    ...
    if (inputType === 'url') {
        ...
        if (fetchExtractError || !extractedContent) {
            ...
            logger.info({ requestId, fetchExtractMs: overallTimings.fetchHtml + overallTimings.extractContent, method: fetchMethodUsed, combinedTextLength: totalLength }, `URL content prepared.`);
