...
// Using Gemini 2.0 Flash rates
const GEMINI_2_0_FLASH_INPUT_COST_PER_1K_TOKENS = 0.0001; // $0.10 per 1M tokens
const GEMINI_2_0_FLASH_OUTPUT_COST_PER_1K_TOKENS = 0.0004; // $0.40 per 1M tokens
...
export function estimateCostUSD(usage: StandardizedUsage, provider: 'gemini' | 'openai'): number {
  ...
  if (provider === 'gemini') {
    inputCost = (usage.inputTokens / 1000) * GEMINI_2_0_FLASH_INPUT_COST_PER_1K_TOKENS;
    outputCost = (usage.outputTokens / 1000) * GEMINI_2_0_FLASH_OUTPUT_COST_PER_1K_TOKENS;
